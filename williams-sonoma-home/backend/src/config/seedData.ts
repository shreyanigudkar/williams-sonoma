...
export const seedDatabase = async () => {
  try {
    ...
    for (const product of sampleProducts) {
      await pool.query(
        `INSERT INTO products (sku_id, product_name, category, sub_category, brand, price, material, color, finish, dimensions, weight, style, description_text)
    ...
    for (const review of sampleReviews) {
      ...
      await pool.query(
        `INSERT INTO reviews (sku_id, customer_id, rating, review_text, review_date)
         VALUES ($1, $2, $3, $4, NOW())`,
        [review.sku_id, customerId, review.rating, review.review_text]
      );
    }
